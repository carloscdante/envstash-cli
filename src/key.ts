...
// save user public key on key create
  ...
  age.stderr.on('data', async (keyBuffer) => {
    ...
    if (typeof userCredentials !== 'boolean') {
      try {
        const { data } = await axios.post('http://localhost:3000/user/update', {
          pubKey,
        }, {
          headers: {
        ...
        return;
          ...
          console.log(`${chalk.red('Error updating personal key on cloud.')} Please log in with ${chalk.bold('envs login')}`)
      }
    }
  });
}
