...
const options = {
  ...
  data: {
    ...
    audience: "https://dev-ibxprky4yw1czvgd.us.auth0.com/api/v2/",
  },
};
  ...
  try {
    ...
    figlet.text(
      ...
      async (err, figletData) => {
        ...
        fs.writeFileSync(
          `${linuxConfigDir}/default-credentials.json`,
...
export const authRequest = async () => {
  try {
    ...
    return await pollForAuthUpdate(data.device_code);
  } catch (error) {
    throw error;
  }
};
...
export const getToken = async (): Promise<Auth | boolean> => {
  ...
  if (!fs.existsSync(`${linuxConfigDir}/default-credentials.json`))
    return false;
  const data: string = fs
    .readFileSync(`${linuxConfigDir}/default-credentials.json`)
    .toString();
